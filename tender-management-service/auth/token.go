...
func (t *TokenManager) CreateToken(accountId int64, role string) (*TokenDetails, error) {
	...
	td.AccessToken, err = at.SignedString([]byte(os.Getenv(utils.AccessSecret)))
	if err != nil {
		return nil, err
	}
	...
	td.RefreshToken, err = rt.SignedString([]byte(os.Getenv(utils.RefreshSecret)))
	if err != nil {
		return nil, err
	}
	return td, nil
...
func VerifyToken(r *http.Request) (*jwt.Token, error) {
	...
	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
		...
		return []byte(os.Getenv(utils.AccessSecret)), nil
	})
	if err != nil {
		return nil, err
	}
